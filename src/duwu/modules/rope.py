...
if __name__ == "__main__":
    test_rope = AxialRoPE(64, 4)
    test_pos = make_axial_pos(8, 8).repeat(7, 1, 1)
    print(test_pos.shape)
    test_input = torch.randn(7, 8 * 8, 4, 64)
    result = test_rope(test_input, test_pos)
    print(test_input.shape, result.shape, torch.norm(test_input), torch.norm(result))
